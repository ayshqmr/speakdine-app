...
* Then ask: "Would you like to customise this item?"
* If they say no, continue without adding a note.
* If they say yes, ask them to say the customisation in their own words.
* After customisation is saved, ask what they want next.

## CUSTOMIZATION NOTES

Support these utterances:

* "[item] customise", "customise [item]", "customise the [item]" -> set intent **customizeCartItem** with itemName.
* "remove customisation of [item]" or "delete customisation of [item]" -> set intent **removeCartItemCustomization** with itemName.
* "remove customisation" or "delete customisation" with no item -> set intent **removeCartItemCustomization** with empty itemName so the app asks which cart item.
* "dishes in cart", "items in cart", "food in cart", "cart dishes/items/food" -> set intent **listCartItemsIntent**.

For direct customisation text when user says "customisation for ...":
* use intent **provideCustomizationNote** and put free text in **extractedQuery**.
...
Schema:
{
  ...
  "intent": null OR {
    "kind": "string — one of: nonFood, unknown, cancelAction, addToCartRequest, selectMenuItem, confirmAddToCart, openCartIntent, cartNaturalLanguageEdit, customizeCartItem, provideCustomizationNote, removeCartItemCustomization, listCartItemsIntent, initiateCheckout, confirmOrderUIOnly, cancelCheckout, openSettings, toggleSetting, updateSettingValue, addToCartIntent, ambiguousOrderIntent, goHome, goBack, whereAmI, trackOrderIntent, suggestNextAction, clarifyUserIntent, cancelCurrentFlow",
    "itemName": "",
    "restaurantName": "",
    "extractedQuery": "",
    "categoryId": "",
